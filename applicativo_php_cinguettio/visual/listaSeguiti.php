...
<!-- ==================== SEZIONE HEADER =========================== -->  
        ...
        <?php 
            ...
            */ 
            ...
            echo '<a href="listaSeguiti.php" class="link-aside-seguiti"> Seguiti'; 
            echo '</a>';
        ...
        <!-- ==================== ELENCO DEGLI UTENTI SEGUITI ==================-->
        <div id="ricerca-utenti-container">
        <?php
            //query
            $utente = $_SESSION['email_utente'];
            $utenti_seguiti = "SELECT * FROM seguaci_seguiti JOIN utente " .
                                    "ON utente_seguito = email_utente WHERE utente_seguace = '{$utente}' AND richiesta_accettata =TRUE";
            $result = mysqli_query($conn, $utenti_seguiti);
                
            if (mysqli_num_rows($result) > 0) {
                while($row = mysqli_fetch_assoc($result)){
                    if ($row['utente_esperto'] == 1){ 
                        echo  
                        /* Smetti di seguire un utente esperto */
                        ...
                        <a href='datiUtente.php?email_utente=" . $row['utente_seguito'] ."'>
                        <img class='img-profilo-dim ' src='../" . $row['path_img_profilo'] ."' alt=''/>
                        </a> 
                        <div class='aside-testo'>{$row['nickname']}</div>
                        <form  action='../validazione/smettiDiSeguireVal.php' method='POST'>
                    ...
                    else if ( $row['utente_esperto'] == 0 ) { 
                        echo 
                        /* smetti di seguire un utente normale */
                        "<div class='profilo-img-container' style='display: inline-block; margin: 0 50px 50px 0;'>
                        <a href='datiUtente.php?email_utente=" . $row['utente_seguito'] ."'>
                        <img class='img-profilo-dim ' src='../" . $row['path_img_profilo'] ."' alt=''/>
                        </a> 
                        <div class='aside-testo'>{$row['nickname']}</div>
                    ...
                    echo '<div>';
                    echo '<h1 class="no-post">Non segui alcun utente </h1>';
                    echo '</div>';
                }
