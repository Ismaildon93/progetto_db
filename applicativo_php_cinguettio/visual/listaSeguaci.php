       ...
       <aside id="aside"> 
        <?php 
            ...
            */ 
            ...
            echo '<a href="listaSeguaci.php" class="link-aside-seguaci"> Seguaci'; 
            echo '</a>';
        ...
        <div id="ricerca-utenti-container">                        
             <?php 
                ...
                $utenti_seguaci = "SELECT * FROM seguaci_seguiti JOIN utente ON "
                                        . "utente_seguace = email_utente WHERE utente_seguito= '{$utente}'";
                $result = mysqli_query($conn, $utenti_seguaci);

                if (mysqli_num_rows($result) > 0) {    
                    while($row = mysqli_fetch_assoc($result)){
                        if  ($row['richiesta_accettata'] == 1 && $row['utente_esperto'] == 1 ){ 
                            echo 
                            ...
                            <div class='richiesta-attesa'> Ti segue dal ";
                             printData($row['data_ora_richiesta'],'d/m/Y');
                             echo "</div>
                            </div></div>"; 
                        }
                        /* utenti esperti che ti seguono con la data di seguito */
                        else if  ($row['richiesta_accettata'] == 1){	
                            echo 
                            " <div class='profilo-img-container' style='display: inline-block; margin: 0 50px 50px 0'>
                            <a href='ricercaUtente.php?email_nickname=" . $row['email_utente'] ."'>
                            <img class='img-profilo-dim ' src='../" . $row['path_img_profilo'] ."' alt=''/>
                            </a> 
                            <div class='aside-testo'> {$row['nickname']}
                            </div>
                            <div class='richiesta-attesa'> Ti segue dal ";
                             printData($row['data_ora_richiesta'],'d/m/Y');
                             echo "</div>
                    ...
                    echo '<div>';
                    echo '<h1 class="no-post">Non hai seguaci </h1>';
                    echo '</div>';
                }
